...
;; remove the region selected constraint
(defun exec-selected-in-haskell ()
  (interactive )
  (if (use-region-p)   (setq haskell-command (format czq-haskell-ghci-multiline(buffer-substring (region-beginning) (region-end)))) 
...
(defun define-haskell-keys ()
  ...
  (define-key haskell-mode-map (kbd "C-c C-k") `get-kind-selected-in-haskell)
  (define-key haskell-mode-map (kbd "C-c C-t") `get-type-selected-in-haskell)
  (define-key haskell-mode-map (kbd "C-c C-m") `get-module-selected-in-haskell)
